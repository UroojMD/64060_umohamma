...
#4(a)Transforming a Categorical Variable

```{r}
#install.packages("caret")
library(caret)
CatVar<- factor(Data$job_title)
#One_hot_Variable<- onehot(CatVar)
#One_hot_Variable?

```
#4(b)Transforming a Quantitative Variable-Zscore, Square root
```{r}
...
ggplot(Data, aes(x = job_tenure, y = base_pay, color = job_tenure)) + geom_point() #scatter plot is plotted using job_tenure as x, base_pay as y and coloured data points from darker to lighter for higher to lower tenure
